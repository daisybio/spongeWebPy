...
def get_network_results(disease_name="Pan-cancer", level="gene"):
    """
    ...
    type_scores = pd.DataFrame(json_dicts['type']['scores']['values'],
                               columns=json_dicts['type']['scores']['labels'])
    type_distances = pd.DataFrame({'x': json_dicts['type']['euclidean_distances']['x'],
    ...
    if len(json_dicts['subtype']) != 0:
        subtype_scores = pd.DataFrame(json_dicts['subtype']['scores']['values'],
                                      columns=json_dicts['subtype']['scores']['labels'])
        subtype_distances = pd.DataFrame({'x': json_dicts['subtype']['euclidean_distances']['x'],
